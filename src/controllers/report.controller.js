...
exports.reportCountCarType = async (req, res) => {
  try {
    ...
    const _countcar = await Order.count({ carType: { $regex: 'ລົດໃຫຍ່' }, status: status }).exec();
    const _countbike = await Order.count({ carType: { $regex: 'ລົດຈັກ' }, status: status }).exec();
    const _countcycle = await Order.count({ carType: { $regex: 'ລົດຖີບ' }, status: status }).exec();
    res.status(200).json({ totalCars: _countcar, totalBike: _countbike, totalCycle: _countcycle });
  } catch (err) {
    console.log(err);
    return res.status(500).json({
      message: `Internal Server Error:${err}`,
...
//     if (findby.carType) {
//       findby.carType = { $regex: findby.carType };
//     }
//     delete findby.skip;
//     delete findby.limit;

//     const _count = await Order.count({ ...findby }).exec();
//     res.status(200).json({ totalCycle: _count });
//   }catch (err) {
//     console.log(err);
//     return res.status(500).json({
//       message: `Internal Server Error:${err}`,
